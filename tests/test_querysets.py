...
def test_with_total_counts_all_entries_per_logger(panel_factory):
    ...
    rows = {
        ...
        for r in Log.objects.values("logger_name").with_total()  # ty: ignore[unresolved-attribute]
    }
    assert rows["app_a"] == 3
    assert rows["app_b"] == 2
...
def test_with_total_errors_counts_error_and_critical(panel_factory):
    ...
    row = Log.objects.values("logger_name").with_total_errors().get()  # ty: ignore[unresolved-attribute]
...
def test_with_total_warnings_counts_only_warnings(panel_factory):
    ...
    row = Log.objects.values("logger_name").with_total_warnings().get()  # ty: ignore[unresolved-attribute]
...
def test_with_recent_errors_includes_entries_after_cutoff(panel_factory):
    ...
    row = (
        Log.objects.values("logger_name")  # ty: ignore[unresolved-attribute]
...
def test_with_last_seen_returns_max_timestamp(panel_factory):
    ...
    row = Log.objects.values("logger_name").with_last_seen().get()  # ty: ignore[unresolved-attribute]
...
def test_cards_aggregation_groups_by_logger_name(panel_factory):
    ...
    rows = list(Log.objects.all().cards_aggregation(one_hour_ago=ONE_HOUR_AGO))  # ty: ignore[unresolved-attribute]
...
def test_cards_aggregation_includes_all_annotations(panel_factory):
    ...
    row = Log.objects.all().cards_aggregation(one_hour_ago=ONE_HOUR_AGO).get()  # ty: ignore[unresolved-attribute]
    for key in (
        "logger_name",
        "total",
        "total_errors",
...
def test_with_has_error_nonzero_when_error_exists(panel_factory):
    ...
    row = Log.objects.values("logger_name").with_has_error().get()  # ty: ignore[unresolved-attribute]
...
def test_with_has_warning_nonzero_when_warning_exists(panel_factory):
    ...
    row = Log.objects.values("logger_name").with_has_warning().get()  # ty: ignore[unresolved-attribute]
...
def test_timeline_aggregation_filters_by_cutoff(panel_factory):
    ...
    rows = list(
        Log.objects.all().timeline_aggregation(  # ty: ignore[unresolved-attribute]
            cutoff=cutoff,
